...
let reportsRowsPerPage = 25;
let reportsSortColumn = 'created_at';
...
function initializeApp() {
    ...
    if (reportsSearchInput) {
        reportsSearchInput.addEventListener('input', debounce(searchReports, 300));
    }

    const reportsRowsPerPageSelect = document.getElementById('reports-rows-per-page');
    if (reportsRowsPerPageSelect) {
        reportsRowsPerPageSelect.value = reportsRowsPerPage.toString();
        reportsRowsPerPageSelect.addEventListener('change', (e) => changeReportsRowsPerPage(parseInt(e.target.value)));
    }
...
function updateReportsTable() {
    ...
    if (reportsSearchTerm) {
        ...
        filtered = allReports.filter(report => {
            ...
            return idStr.includes(term) ||
                ...
                (report.type_display || report.type || '').toLowerCase().includes(term) ||
    ...
    const sortedReports = [...filtered].sort((a, b) => {
        let aVal = a[reportsSortColumn] || '';
        let bVal = b[reportsSortColumn] || '';
        if (reportsSortColumn === 'id') { aVal = parseInt(a.id) || 0; bVal = parseInt(b.id) || 0; }
        if (reportsSortColumn === 'reporter') {
            aVal = a.reporter || 'Anonymous';
            bVal = b.reporter || 'Anonymous';
            if (typeof aVal === 'string' && aVal.includes('@')) {
                const user = allUsers.find(u => u.email?.toLowerCase() === aVal.toLowerCase());
                if (user && (user.full_name || user.name)) aVal = user.full_name || user.name;
            }
            if (typeof bVal === 'string' && bVal.includes('@')) {
                const user = allUsers.find(u => u.email?.toLowerCase() === bVal.toLowerCase());
                if (user && (user.full_name || user.name)) bVal = user.full_name || user.name;
            }
        }
        if (reportsSortColumn === 'type') { aVal = a.type_display || a.type || ''; bVal = b.type_display || b.type || ''; }
        if (reportsSortColumn === 'location') {
            aVal = a.location || '';
            bVal = b.location || '';
        }
        if (reportsSortColumn === 'created_at') { aVal = new Date(a.created_at || 0).getTime(); bVal = new Date(b.created_at || 0).getTime(); }
        if (reportsSortColumn === 'status') { aVal = a.status || 'pending'; bVal = b.status || 'pending'; }
        if (aVal < bVal) return reportsSortDirection === 'asc' ? -1 : 1;
        if (aVal > bVal) return reportsSortDirection === 'asc' ? 1 : -1;
        return 0;
    });
    
    const startIdx = (reportsCurrentPage - 1) * reportsRowsPerPage;
    const pageReports = sortedReports.slice(startIdx, startIdx + reportsRowsPerPage);
    ...
    updateReportsPagination(filtered.length);
}

function updateReportsPagination(total) {
    const showingEl = document.getElementById('reports-showing-count');
    const totalEl = document.getElementById('reports-total-count');
    if (showingEl) showingEl.textContent = Math.min(total, reportsCurrentPage * reportsRowsPerPage);
    if (totalEl) totalEl.textContent = total;

    const prevBtn = document.getElementById('reports-prev-btn');
    const nextBtn = document.getElementById('reports-next-btn');
    const pageNum = document.getElementById('reports-page-num');
    const totalPages = Math.ceil(total / reportsRowsPerPage) || 1;
    if (prevBtn) prevBtn.disabled = reportsCurrentPage === 1;
    if (nextBtn) nextBtn.disabled = reportsCurrentPage >= totalPages;
    if (pageNum) pageNum.textContent = reportsCurrentPage;
}
...
function updateRespondersTable() {
    ...
    const prevBtn = document.getElementById('responders-prev-btn');
    const nextBtn = document.getElementById('responders-next-btn');
    const pageNum = document.getElementById('responders-page-num');
    const totalPages = Math.ceil(allResponders.length / rowsPerPage) || 1;
...
/* ---------------- REPORTS PAGINATION & SEARCH ---------------- */
function changeReportsRowsPerPage(value) {
    reportsRowsPerPage = parseInt(value);
    reportsCurrentPage = 1;
    updateReportsTable();
}

function nextReportsPage() {
    const total = reportsSearchTerm ? getFilteredReportsCount() : allReports.length;
    if (reportsCurrentPage < Math.ceil(total / reportsRowsPerPage)) {
        reportsCurrentPage++;
        updateReportsTable();
    }
}
...
function getFilteredReportsCount() {
    if (!reportsSearchTerm) return allReports.length;
    const term = reportsSearchTerm.toLowerCase();
...
function sortReportsTable(column) {
    if (reportsSortColumn === column) reportsSortDirection = reportsSortDirection === 'asc' ? 'desc' : 'asc';
    else { reportsSortColumn = column; reportsSortDirection = 'asc'; }
    updateReportsTable();
}

function searchReports(event) {
    reportsSearchTerm = event.target.value.trim();
    reportsCurrentPage = 1;
    updateReportsTable();
}
...
function exportReports() {
    ...
    const headers = ['ID', 'Reporter', 'Type', 'Location', 'Time', 'Status'];
    const rows = allReports.map(r => {
        ...
        let location = 'Unknown location';
        if (r.location && r.location.trim() !== '') location = r.location;
        else if (r.latitude && r.longitude) {
            const lat = parseFloat(r.latitude), lng = parseFloat(r.longitude);
            if (!isNaN(lat) && !isNaN(lng)) location = `${lat.toFixed(4)}, ${lng.toFixed(4)}`;
        }
        return [r.id, reporterName, r.type_display || r.type || 'Emergency', location, r.created_at ? new Date(r.created_at).toLocaleString() : '', r.status || 'pending'];
    });
    ...
    logAudit(currentUser?.email || 'Admin', 'export_data', 'Exported reports to CSV', 'reports');
}
...
window.changeReportsRowsPerPage = changeReportsRowsPerPage;
